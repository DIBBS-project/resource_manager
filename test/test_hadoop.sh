...
curl --header "token: $TOKEN" -i -X POST -F 'data=@test.txt' $REMOTE_HADOOP_WEBSERVICE_HOST/hdfs/upload/user/$USER/input/

# Create Hadoop job
HADOOP_JOB_CREATION_OUTPUT=$(curl --header "token: $TOKEN" -H "Content-Type: application/json" -X POST -d "{\"name\": \"test\", \"command\": \"test.jar input output dummyparameter\", \"callback_url\": \"$CALLBACK_URL\"}" $REMOTE_HADOOP_WEBSERVICE_HOST/jobs/)
