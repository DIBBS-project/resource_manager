...
class MisterClusterHeat(MisterClusterInterface):
    ...
    def resize_cluster(self, cluster, new_size=1, master=None):
        ...
        if stack_id is not None:
            resource_group = None
            while resource_group is None or resource_group.physical_resource_id == "":
                fields = {
                    "stack_id": stack_id,
                    "nested_depth": None,
                    "with_detail": False,
...
#     def provision_new_instance(self, client, host, image_name, user_data=None):
#
...
#
#         instance = client.servers.create(instance_name, image, flavor, userdata=user_data)
#
#         if host.name == "":
...
#             instance = client.servers.find(id=instance.id)
#             time.sleep(1)
#
...
#         get_an_available_floating_ip = lambda: client.floating_ips.findall(instance_id=None)
...
#
#         client.servers.add_floating_ip(instance, floating_ip)
